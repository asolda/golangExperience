package lexer;

public class sym {
	public static final int EOF = 0;
	public static final int DIV = 3; 
	public static final int TIME = 4;
	public static final int OCTAL_LITERAL = 5;
	public static final int EQ = 7;
	public static final int PLUS = 8;
	public static final int MINUS = 9;
	public static final int AND = 10;
	public static final int NOT = 11;
	public static final int RO = 12;
	public static final int RC = 13;
	public static final int OR = 14;
	public static final int AO = 15;
	public static final int OS = 16;
	public static final int CS = 17;
	public static final int CAP = 18;
	public static final int AC = 19;
	public static final int BRACEO = 20;
	public static final int BRACEC = 21;
	public static final int TP = 22;
	public static final int COMMA = 23;
	public static final int SEMICOLON = 24;
	public static final int PERC = 25;
	public static final int POINT = 26;
	public static final int STRING_LITERAL = 28;
	public static final int DIVEQ = 30;
	public static final int TIMEEQ = 31;
	public static final int IF = 32;
	public static final int GO = 33;
	public static final int EQEQ = 34;
	public static final int PLUSEQ = 35;
	public static final int PLUSPLUS = 36;
	public static final int MINUSEQ = 37;
	public static final int MINUSMINUS = 38;
	public static final int ANDEQ = 39;
	public static final int ANDAND = 40;
	public static final int ANDCAP = 41;
	public static final int NOTEQ = 42;
	public static final int OREQ = 43;
	public static final int OROR = 44;
	public static final int AOMINUS = 45;
	public static final int AOAO = 46;
	public static final int CAPEQ = 47;
	public static final int AOEQUALS = 48;
	public static final int ACAC = 49;
	public static final int TPEQ = 50;
	public static final int PERCEQ = 51;
	public static final int FOR = 56;
	public static final int MAP = 57;
	public static final int DECIMAL_LITERAL = 58;
	public static final int ANDCAPEQ = 59;
	public static final int AOAOEQ = 60;
	public static final int ACACEQ = 61;
	public static final int POINTPOINTPOINT = 62;
	public static final int VAR = 63;
	public static final int FUNC = 64;
	public static final int ELSE = 65;
	public static final int TYPE = 66;
	public static final int CASE = 67;
	public static final int CHAN = 68;
	public static final int GOTO = 69;
	public static final int IDENTIFIER = 70;
	public static final int RANGE = 71;
	public static final int DEFER = 72;
	public static final int CONST = 73;
	public static final int BREAK = 74;
	public static final int RETURN = 75;
	public static final int IMPORT = 76;
	public static final int SELECT = 77;
	public static final int SWITCH = 78;
	public static final int DEFAULT = 79;
	public static final int INTEGER_LITERAL = 80;
	public static final int PACKAGE = 81;
	public static final int CONTINUE = 82;
	public static final int INTERFACE = 83;
	public static final int HEX_LITERAL = 84;
	public static final int FALLTHROUGH = 85;
	
}
